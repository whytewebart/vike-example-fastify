...
type BroCliConfig<TOpts extends Record<string, GenericBuilderInternalsLimited> | undefined = undefined, TOptsData = TOpts extends Record<string, GenericBuilderInternals> ? TypeOf<TOpts> : undefined> = {
    name?: string;
    description?: string;
    argSource?: string[];
    ...
    globals?: TOpts;
    hook?: (event: EventType, command: Command, options: TOptsData) => any;
 ...
 */
declare const run: <TOpts extends Record<string, GenericBuilderInternalsLimited> | undefined = undefined>(commands: Command[], config?: BroCliConfig<TOpts>) => Promise<unknown>;
 ...
 */
type EventHandler<TOpts = unknown> = (event: BroCliEvent, 
/** Global options. `undefined` if globals failed to parse. */
options?: TOpts | undefined) => boolean | Promise<boolean>;
