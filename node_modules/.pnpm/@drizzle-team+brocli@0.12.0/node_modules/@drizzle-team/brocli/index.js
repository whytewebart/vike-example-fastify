...
var validateGlobalsInner = (commands, globals) => {
  for (const c of commands) {
    ...
    for (const { config: opt } of Object.values(options)) {
      ...
      if (foundAliasOverlap) {
        throw new BroCliError(
          `Global options overlap with option '${opt.name}' of command '${getCommandNameWithParents(c)}' on alias '${foundAliasOverlap}'`
        );
      }
    }
...
var run = async (commands, config) => {
  ...
  try {
    ...
    if (!args.length) {
      return help !== void 0 ? await executeOrLog(help, preparsedGlobals()) : await eventHandler({
        type: "global_help",
        description: cliDescription,
        name: cliName,
    ...
    if (versionIndex !== -1 && (versionIndex > 0 ? args[versionIndex - 1]?.startsWith("-") ? false : true : true)) {
      return version !== void 0 ? await executeOrLog(version, preparsedGlobals()) : await eventHandler({
        type: "version",
        name: cliName,
        description: cliDescription
      }, preparsedGlobals());
    ...
    const optionResult = gOptionResult && (gOptionResult === "help" || gOptionResult === "version") ? gOptionResult : parseOptions(
      command2,
      globals ? newArgs.filter((a) => a !== void 0) : newArgs,
      cliName,
      cliDescription,
      omitKeysOfUndefinedOptions
    );
