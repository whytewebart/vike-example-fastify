	...
	*/
	buildSelection(fields, { isSingleTable = false } = {}) {
		...
		const chunks = fields.flatMap(({ field }, i) => {
			...
			else if ((0, __entity_ts.is)(field, __subquery_ts.Subquery)) if (!field._.isWith) chunk.push(__sql_sql_ts.sql`(${field._.sql}) ${__sql_sql_ts.sql.identifier(field._.alias)}`);
	...
	buildSelectionOutput(fields, { type }) {
		...
		const chunks = fields.flatMap(({ field }, i) => {
			const chunk = [];
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased)) if (field.isSelectionField) chunk.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), __sql_sql_ts.sql.identifier(field.fieldAlias)]));
			else {
				const query = field.sql;
	...
	buildSelectQuery({ withList, fieldsFlat, where, having, table, joins, orderBy, groupBy, fetch, for: _for, top, offset, distinct, setOperators }) {
