...
type PgInsertSelection<TTable extends PgTable<TableConfig$1>, OverrideT extends boolean = false, TModel extends Record<string, unknown> = InferInsertModel<TTable, {
  override: OverrideT;
}>> = { [K in keyof TModel]: AnyPgColumn | SQL | SQL.Aliased | TModel[K] } & {};
type NoUnknownKeysInInsertSelection<TTable extends PgTable<TableConfig$1>, TSelection extends PgInsertSelection<any, any>, OverrideT extends boolean = false> = { [K in keyof TSelection]: K extends keyof InferInsertModel<TTable, {
  override: OverrideT;
}> ? TSelection[K] : K extends keyof InferInsertModel<TTable, {
  override: true;
}> ? DrizzleTypeError<`Column "${K & string}" in table "${TTable['_']['name']}" is a generated column. Use \`.overridingSystemValue()\` to manually insert into this column`> : DrizzleTypeError<`Column "${K & string}" does not exist in table "${TTable['_']['name']}"`> };
...
declare class PgInsertBuilder<TTable extends PgTable, TQueryResult extends PgQueryResultHKT, OverrideT extends boolean = false, TBuilderHKT extends PgInsertHKTBase = PgInsertHKT> {
  ...
  select<TSelection extends PgInsertSelection<TTable, OverrideT>>(selectQuery: (qb: QueryBuilder) => TypedQueryBuilder<NoUnknownKeysInInsertSelection<TTable, TSelection, OverrideT>>): PgInsertKind<TBuilderHKT, TTable, TQueryResult>;
  ...
  select<TSelection extends PgInsertSelection<TTable, OverrideT>>(selectQuery: TypedQueryBuilder<NoUnknownKeysInInsertSelection<TTable, TSelection, OverrideT>>): PgInsertKind<TBuilderHKT, TTable, TQueryResult>;
}
...
//#endregion
export { AnyPgInsert, NoUnknownKeysInInsertSelection, PgInsert, PgInsertBase, PgInsertBuilder, PgInsertBuilderConstructor, PgInsertConfig, PgInsertDynamic, PgInsertHKT, PgInsertHKTBase, PgInsertKind, PgInsertOnConflictDoUpdateConfig, PgInsertReturning, PgInsertReturningAll, PgInsertSelection, PgInsertValue, PgInsertWithout };
