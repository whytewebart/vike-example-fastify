	...
	*/
	buildSelection(fields, { isSingleTable = false, ignoreCastCodecs = false } = {}) {
		...
		const chunks = fields.flatMap(({ field, codecOverride, column }, i) => {
			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased)) if (field.isSelectionField) {
				...
				chunk.push(__sql_sql_ts.sql` as ${__sql_sql_ts.sql.identifier(field.fieldAlias)}`);
			}
			...
			} else if ((0, __entity_ts.is)(field, __subquery_ts.Subquery)) if (column && !ignoreCastCodecs && !field._.isWith) {
	...
	buildSetOperations(leftSelect, leftSelection, ignoreSelectionCastCodecs, setOperators) {
		...
		for (let i = 0; i < setOperators.length; ++i) {
			...
			const rightSelection = (0, __utils_ts.orderSelectedFields)(setOperator.rightSelect.getSelectedFields());
			for (let j = 0; j < outputSelection.length; ++j) {
				...
				outputSelection[j].codecOverride = lc && rc ? require_pg_core_codecs.unionsTypeTable[lc]?.[rc] : lc;
			}
		}
		...
		return ignoreSelectionCastCodecs ? leftSelect : __sql_sql_ts.sql`select ${this.buildSelection(outputSelection.map((field) => {
			if ((0, __entity_ts.is)(field.field, __sql_sql_ts.SQL.Aliased)) {
			...
			if ((0, __entity_ts.is)(field.field, __column_ts.Column) && field.field.isAlias) {
			...
			if ((0, __entity_ts.is)(field.field, __subquery_ts.Subquery)) {
	...
	buildInsertQuery({ table, values: valuesOrSelect, onConflict, returning, withList, select, overridingSystemValue_, comment, ignoreSelectionCastCodecs }) {
		...
		const insertOrder = colFilteredEntries.map(([, column]) => __sql_sql_ts.sql.identifier(column.name));
		if (select) {
