...
const upgradeFunctions = { 0: (migrationsTable, session, localMigrations) => {
	return Effect.gen(function* () {
		...
		yield* session.execute(sql`ALTER TABLE ${table} ADD ${sql.identifier("applied_at")} TIMESTAMP DEFAULT CURRENT_TIMESTAMP`);
		for (const backfillEntry of toApply) yield* session.execute(sql`UPDATE ${table} SET ${sql.identifier("name")} = ${backfillEntry.name}, ${sql.identifier("applied_at")} = NULL WHERE ${sql.identifier("id")} = ${backfillEntry.id}`);
	});
} };
/**
...
const upgradeIfNeeded = Effect.fn("upgradeIfNeeded")(function* (migrationsTable, session, localMigrations) {
	...
	const rows = yield* session.objects(sql`SELECT column_name as \`column_name\`
...
//# sourceMappingURL=effect-mysql.js.map