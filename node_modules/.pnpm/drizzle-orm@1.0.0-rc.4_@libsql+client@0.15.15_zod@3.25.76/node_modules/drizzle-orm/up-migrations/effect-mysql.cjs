...
//#region src/up-migrations/effect-mysql.ts
/**
...
const upgradeFunctions = { 0: (migrationsTable, session, localMigrations) => {
	return effect.Effect.gen(function* () {
		...
		yield* session.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD ${__sql_sql_ts.sql.identifier("name")} text`);
		yield* session.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD ${__sql_sql_ts.sql.identifier("applied_at")} TIMESTAMP DEFAULT CURRENT_TIMESTAMP`);
		for (const backfillEntry of toApply) yield* session.execute(__sql_sql_ts.sql`UPDATE ${table} SET ${__sql_sql_ts.sql.identifier("name")} = ${backfillEntry.name}, ${__sql_sql_ts.sql.identifier("applied_at")} = NULL WHERE ${__sql_sql_ts.sql.identifier("id")} = ${backfillEntry.id}`);
	});
} };
/**
...
const upgradeIfNeeded = effect.Effect.fn("upgradeIfNeeded")(function* (migrationsTable, session, localMigrations) {
	...
	const rows = yield* session.objects(__sql_sql_ts.sql`SELECT column_name as \`column_name\`
...
//# sourceMappingURL=effect-mysql.cjs.map