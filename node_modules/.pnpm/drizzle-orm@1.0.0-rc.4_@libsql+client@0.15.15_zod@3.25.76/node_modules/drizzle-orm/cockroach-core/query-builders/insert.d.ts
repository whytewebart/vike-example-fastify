...
type CockroachInsertSelection<TTable extends CockroachTable<TableConfig$1>, TModel extends Record<string, unknown> = InferInsertModel<TTable>> = { [K in keyof TModel]: AnyCockroachColumn | SQL | SQL.Aliased | TModel[K] } & {};
type NoUnknownKeysInInsertSelection<TTable extends CockroachTable<TableConfig$1>, TSelection extends CockroachInsertSelection<any>> = { [K in keyof TSelection]: K extends keyof InferInsertModel<TTable> ? TSelection[K] : K extends keyof InferInsertModel<TTable, {
  override: true;
...
declare class CockroachInsertBuilder<TTable extends CockroachTable, TQueryResult extends CockroachQueryResultHKT, OverrideT extends boolean = false> {
  ...
  select<TSelection extends CockroachInsertSelection<TTable>>(selectQuery: (qb: QueryBuilder) => TypedQueryBuilder<NoUnknownKeysInInsertSelection<TTable, TSelection>>): CockroachInsertBase<TTable, TQueryResult>;
  ...
  select<TSelection extends CockroachInsertSelection<TTable>>(selectQuery: TypedQueryBuilder<NoUnknownKeysInInsertSelection<TTable, TSelection>>): CockroachInsertBase<TTable, TQueryResult>;
}
