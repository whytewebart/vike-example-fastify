export default {
    root: ({ props, context, parent, instance, attrs }) => {
        function isReflect(hover) {
            const exists = Object.hasOwn(attrs, "reflect") && !props.link;
            if (!hover) return exists && attrs.reflect === "";
            if (hover) return exists && attrs.reflect === "hover";
        }

        function checkAttr(attr, val) {
            const exists = Object.hasOwn(attrs, attr);
            if (val) return attrs[attr] === val && exists;
            return exists;
        }
        ...
        return {
            class: [
                ...
                {
                    "shadow-reflect": isReflect(),
                    "hover:shadow-reflect hover:bg-transparent":
                        isReflect(true) && props.outlined,
                    "hover:shadow-reflect": isReflect(true),
                },

                {
                    // "w-full": checkAttr("block"),
                    "w-full": props.fluid,
                },
    ...
    label: ({ props }) => ({
        class: [
            ...
            { 'flex-1': props.label !== null && props.fluid === null, 'invisible w-0': props.label == null }
        ]
    }),
    icon: ({ props }) => ({
        class: [
